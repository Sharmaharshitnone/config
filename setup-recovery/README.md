...
## 🔄 Other Machine / Update-Only Scenario

Use this when you **already have everything installed** on a second machine and just cloned the dotfiles to a **different directory** than the original machine (e.g., `/home/user/dotfiles/config` instead of `/home/kali/work/config`).

**Do NOT run the full bootstrap** — packages are already installed. You only need to fix the machine-specific paths.

### What changes between machines

Two categories of path-sensitive data:

| Item | Problem | Fix |
|------|---------|-----|
| `/etc/sudoers.d/kb-rgb` | NOPASSWD rule hardcodes the old clone path | Regenerated by `update-paths.sh` |
| `/etc/sudoers.d/warp` | Same — wrong canonical path → password prompt | Regenerated by `update-paths.sh` |
| `dunst/scripts/kb-rgb-glow` | `exec sudo /old/path/kb-rgb glow-flash` | Patched in-place |
| `i3/config.d/keyboard-rgb.conf` | `set $kb /old/path/kb-rgb` | Patched in-place |
| `~/.local/bin/*` symlinks | Point to old clone location | Re-symlinked by `update-paths.sh` |

### One-command fix

```bash
cd /path/to/cloned/config/setup-recovery
...
# Restart dunst (picks up patched glow hook)
pkill dunst && dunst &
...
### Why this works (principal engineer note)

The core problem: `sudo` NOPASSWD rules must contain the **canonical realpath** of the binary — not a symlink. When you clone to a different directory the canonical path changes, so the old sudoers entry stops matching and sudo falls back to asking for a password.

`update-paths.sh` runs `realpath` on the scripts **in the current clone location** and writes fresh sudoers rules from that result. Every subsequent `sudo` invocation resolves the symlink chain to the same canonical path → NOPASSWD match → no prompt.

The `warp` and `kb-rgb` scripts themselves also do `realpath "${BASH_SOURCE[0]}"` before calling `exec sudo "$_SELF"`, so they self-correct even if launched via symlink.

### Full restore-dotfiles also handles this

`restore-dotfiles.sh` runs the same path-patching logic at the end of every run, so if you ever run the full setup on a new machine it automatically configures the correct paths for that machine's clone location.
